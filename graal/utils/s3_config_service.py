...
class S3ConfigService:
    ...
    def _initialize_s3(self) -> bool:
        ...
        required_vars = ['S3_BUCKET_NAME', 'S3_ENDPOINT', 'S3_ACCESS_KEY', 'S3_SECRET_KEY']
    ...
    def _download_from_s3(self, s3_key: str) -> BytesIO:
        ...
        try:
            logger.info(f"Downloading configuration file from S3: s3://{bucket_name}/{s3_key}")
    ...
    def load_config_excel(self, filename: str) -> dict[str, pd.DataFrame]:
        ...
        if self._s3_enabled:
            ...
            if s3_key:
                try:
                    ...
                    logger.info(f"Loaded configuration from S3 - sheets: {list(excel_data.keys())}")
                    return excel_data
                except Exception as e:
                    logger.warning(f"Failed to load from S3, falling back to local: {e}")
            else:
        ...
        if not local_path.exists():
            raise FileNotFoundError(f"Configuration file not found locally: {local_path}")
            
        try:
            logger.info(f"Loading configuration from local file: {local_path}")
            excel_data = pd.read_excel(local_path, sheet_name=None)
            logger.info(f"Loaded configuration from local file - sheets: {list(excel_data.keys())}")
            return excel_data
        except Exception as e:
            raise Exception(f"Failed to load local configuration file {local_path}: {e}")

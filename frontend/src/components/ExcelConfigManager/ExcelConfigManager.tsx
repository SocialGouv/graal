...
export const ExcelConfigManager: React.FC = () => {
  ...
  const renderRoleBadge = (role: ExcelConfigManifest['current_user_role']) => {
    if (role === 'owner') {
      return (
  ...
  const canManageConfig = (config: ExcelConfigManifest) =>
    config.current_user_role === 'owner'

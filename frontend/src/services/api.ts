...
class ApiService {
  ...
  /**
   * List all Excel config manifests (admin only)
   */
  async listAdminExcelConfigs(): Promise<ExcelConfigListResponse> {
    console.log('[API_CLIENT] Fetching admin Excel configs')

    try {
      const response = await this.client.get<ExcelConfigListResponse>(
        '/admin/excel-configs'
      )

      console.log('[API_CLIENT] Admin Excel configs retrieved', {
        total: response.data.total
      })

      return response.data
    } catch (error) {
      console.error('[API_CLIENT] Failed to fetch admin Excel configs', error)
      throw error
    }
  }
  ...
  /**
   * Delete an Excel config as admin
   */
  async deleteAdminExcelConfig(configId: string): Promise<void> {
    console.log('[API_CLIENT] Deleting admin Excel config', { configId })

    try {
      await this.client.delete(`/admin/excel-configs/${configId}`)
      console.log('[API_CLIENT] Admin Excel config deleted', { configId })
    } catch (error) {
      console.error('[API_CLIENT] Failed to delete admin Excel config', error)
      throw error
    }
  }
  ...
  /**
   * Delete a similarity database and its backing S3 file by manifest ID (owner or admin).
   */
  async deleteDatabaseForOwner(id: string): Promise<void> {
    console.log('[API_CLIENT] Deleting database as owner', { id })

    try {
      await this.client.delete(`/databases/${id}`)

      console.log('[API_CLIENT] Database deleted', { id })
    } catch (error) {
      console.error('[API_CLIENT] Failed to delete database', { id }, error)
      throw error
    }
  }

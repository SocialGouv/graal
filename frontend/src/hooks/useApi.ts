...
export const useUploadFile = () => {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      const errorMessage = error.detail || 'Erreur lors du téléchargement du fichier';
...
export const useJobStatus = (jobId: string | null, enabled: boolean = true) => {
  ...
  React.useEffect(() => {
    if (query.error) {
      const errorMessage = (query.error as any).detail || 'Erreur lors de la récupération du statut';
      setError(errorMessage);
    }
...
export const useResultsPreview = (jobId: string | null, enabled: boolean = false) => {
  ...
  React.useEffect(() => {
    if (query.error) {
      const errorMessage = (query.error as any).detail || 'Erreur lors de la récupération des résultats';
      setError(errorMessage);
    }
  }, [query.error, setError]);

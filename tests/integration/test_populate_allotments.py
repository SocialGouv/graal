...
def load_test_file_to_compare(excel_test_file_path: str, sheet_name: str):
    data_extractor = PLFSSSheetDataLoader(excel_test_file_path)
    return data_extractor.extract_sheet_data(sheet_name)


def test_populate_allotments_ratio_matching_allotments():
    ...
    merged_df = test_df.merge(
        amendments_df, on=["Num article", "Num amdt"], suffixes=("_test", "_algo")
    )
